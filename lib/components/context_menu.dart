...
Rect _getRect(GlobalKey globalKey) {
  ...
  final containerOffset = renderBoxContainer.localToGlobal(
...
class _CupertinoContextMenuState extends State<CupertinoContextMenu>
  ...
  _ContextMenuLocation get _contextMenuLocation {
    final childRect = _getRect(_childGlobalKey);
    final screenWidth = MediaQuery.of(context).size.width;

    final center = screenWidth / 2;
    final centerDividesChild =
        childRect.left < center && childRect.right > center;
    final distanceFromCenter = (center - childRect.center.dx).abs();
...
class _DecoyChildState extends State<_DecoyChild>
  ...
  void initState() {
    ...
    final midRect = widget.beginRect!.deflate(
  ...
  Widget _buildAnimation(BuildContext context, Widget? child) {
    final color = widget.controller.status == AnimationStatus.reverse
        ? _masklessColor
        : _mask.value;
...
class _ContextMenuRoute<T> extends PopupRoute<T> {
  ...
  static Rect _getScaledRect(GlobalKey globalKey, double scale) {
    final childRect = _getRect(globalKey);
    final sizeScaled = childRect.size * scale;
  ...
  static Rect _getSheetRectBegin(
    ...
    switch (contextMenuLocation) {
      case _ContextMenuLocation.center:
        final target = orientation == Orientation.portrait
            ? childRect.bottomCenter
            : childRect.topCenter;
        final centered = target - Offset(sheetRect.width / 2, 0.0);
  ...
  void _updateTweenRects() {
    ...
    final sheetRect = _getRect(_sheetGlobalKey);
    final sheetRectBegin = _getSheetRectBegin(
      _lastOrientation,
    ...
    return OrientationBuilder(
      builder: (BuildContext context, Orientation orientation) {
        ...
        if (!animation.isCompleted) {
          ...
          final sheetRect = reverse
              ? _sheetRectAnimatableReverse.evaluate(animation)!
              : _sheetRectAnimatable.evaluate(animation)!;
          final sheetScale = reverse
...
class _ContextMenuRouteStaticState extends State<_ContextMenuRouteStatic>
  ...
  void _setDragOffset(Offset dragOffset) {
    ...
    final endX = _kPadding * dragOffset.dx / _kDamping;
    final endY = dragOffset.dy >= 0.0
        ? dragOffset.dy
  ...
  Rect _getPreViewRect(Size screenSize) {
    ...
    if (widget.previewSize == null && widget.previewOffset == null) {
      ...
      retRect =
          Offset(widget.childRectOriginal.left, widget.childRectOriginal.top) &
              widget.previewSize!;
    } else {
  ...
  Widget _getChild(RelativeRect position, Size screenSize) {
    ...
    if (position.right < _kPadding) dx = dx - (_kPadding - position.right);
    ...
    if (position.bottom < _kPadding) dy = dy - (_kPadding - position.bottom);
    ...
    _childBottom = math.max(
        0.0, screenSize.height - 2.0 * _kPadding - _childTop - _childHeight);
    _childRight = math.max(
        0.0, screenSize.width - 2.0 * _kPadding - _childLeft - _childWidth);
  ...
  void _calcMenuSheetOffset(
    ...
    if (orientation == Orientation.portrait) {
      if (_childTop <= _childBottom) {
        ...
        _menuSheetBottom =
            math.max(0.0, screenSize.height - _childTop - _kPadding);
        _menuSheetTop = 0.0;
      }
      ...
      if (_childTop <= _childBottom) {
        ...
        _alignment = AlignmentDirectional.bottomStart;
        _menuSheetTop = 0.0;
        _menuSheetBottom = math.max(0.0,
            screenSize.height - _childTop - 2.0 * _kPadding - _childHeight);
      }
  ...
  double _getMenuSheetWidth(
    Orientation orientation,
    Size screenSize,
  ) {
    if (orientation == Orientation.portrait) {
      final width = math.max(_childLeft, _childRight) + _childWidth;
      return math.min(width, screenSize.width * 2.0 / 3.0);
    }
    final width =
        math.min(math.max(_childLeft, _childRight), screenSize.width / 2.0) -
            2.0 * _kPadding;
  ...
  List<Widget> _getChildren(
    ...
    final child = _getChild(position, screenSize);
    final sheetWidth = _getMenuSheetWidth(orientation, screenSize);
