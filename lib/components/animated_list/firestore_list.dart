...
typedef DocumentCallback = void Function(
    int index, List<DocumentSnapshot?> snapshot);
typedef FilterCallback = bool Function(DocumentSnapshot snapshot);
typedef ValueCallback = void Function(DocumentSnapshot snapshot);
typedef QueryCallback = void Function(QuerySnapshot querySnapshot);
...
class FirestoreList extends ListBase<DocumentSnapshot?>
  ...
  void _onDocumentAdded(DocumentChange event) {
    try {
      ...
      if (linear) {
        ...
        onDocumentAdded?.call(_snapshots.length - 1, _snapshots);
      } else {
        final index = event.newIndex >= length ? length : event.newIndex;
        _snapshots.insert(index, event.doc);
        onDocumentAdded?.call(index, _snapshots);
      }
    } catch (error) {
  ...
  void _onDocumentRemoved(DocumentChange event) {
    try {
      ...
      if (index > -1) {
        ...
        onDocumentRemoved?.call(index, _snapshots);
      } else {
  ...
  void _onDocumentChanged(DocumentChange event) {
    ...
    if (index > -1) {
      ...
      onDocumentChanged?.call(index, _snapshots);
    }
  }
