...
///
/// A [MBackButton] is an [IconButton] with a "back" icon appropriate for the
/// current [TargetPlatform]. When pressed, the back button calls
/// [Navigator.maybePop] to return to the previous route unless a custom
/// [onPressed] callback is provided.
///
/// When deciding to display a [MBackButton], consider using
...
/// initial route), the [MBackButton] will not have any effect when pressed,
...
/// See also:
///
///  * [AppBar], which automatically uses a [MBackButton] in its
...
class MBackButton extends StatelessWidget {
  ...
  /// target platform.
  const MBackButton({super.key, this.color, this.onPressed});
...
/// widget whose child is an [MIconButton]. The icon button's filled background
...
/// {@end-tool}
///
/// Material Design 3 introduced new types (standard and contained) of [MIconButton]s.
/// The default [MIconButton] is the standard type, and contained icon buttons can be produced
/// by configuring the [MIconButton] widget's properties.
///
/// {@tool dartpad}
/// This sample shows creation of [MIconButton] widgets for standard, filled,
...
///  * [MBackButton], an icon button for a "back" affordance which adapts to the
...
class MIconButton extends StatelessWidget {
  ...
  /// [Icon.size] property allows the [MIconButton] to size the splash area to
  /// fit the [Icon]. If you were to set the size of the [Icon] using
  /// [Icon.size] instead, then the [MIconButton] would default to 24.0 and then
  ...
  ///
  /// The [style] is only used for Material 3 [MIconButton]. If [ThemeData.useMaterial3]
  /// is set to true, [style] is preferred for icon button customization, and any
  /// parameters defined in [style] will override the same parameters in [MIconButton].
  ///
  /// For example, if [MIconButton]'s [visualDensity] is set to [VisualDensity.standard]
  ...
  /// [MIconButton], as well as its overlay color, with all of the
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveIconSize = iconSize ?? IconTheme.of(context).size ?? 24.0;

    if (theme.useMaterial3) {
      final minSize = constraints == null
          ? null
          : Size(constraints!.minWidth, constraints!.minHeight);
      final maxSize = constraints == null
          ? null
      ...
      var iconButton = IconTheme.merge(
        data: IconThemeData(
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
      ..add(DiagnosticsProperty<Widget>('icon', icon, showName: false))
      ..add(
...
class _IconButtonM3 extends ButtonStyleButton {
  ...
  /// Returns null because [MIconButton] doesn't have its component theme.
  @override
